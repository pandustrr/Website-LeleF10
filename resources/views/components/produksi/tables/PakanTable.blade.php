<div class="flex flex-col lg:flex-row gap-6 w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($pakans as $index => $pakan)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-base whitespace-nowrap">
                            {{ number_format($pakan->kuantitas) }} kg
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div class="flex justify-end gap-3">
                                ...
                                <button onclick="openEditModal('{{ route('pakan.edit', $pakan->id) }}', 'pakan')"
                                ...
                                <!-- Tombol Hapus -->
                                <form action="{{ route('pakan.destroy', $pakan->id) }}" method="POST" class="inline delete-form">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-6 text-center text-gray-500 text-lg">
                            ...
                            <p class="mt-4">Belum ada data pakan</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div class="w-full lg:w-64 bg-green-500 text-white p-4 rounded-lg shadow-md">
        <h3 class="text-lg font-semibold mb-3">Total Pakan</h3>
        <div class="space-y-3">
            <div>
                <div class="text-sm font-medium mb-1">Total (kg)</div>
                <div class="bg-white/20 p-2 rounded-lg text-xl font-bold">
                    {{ number_format($pakans->sum('kuantitas')) }}
                </div>
            </div>
        </div>
    </div>
